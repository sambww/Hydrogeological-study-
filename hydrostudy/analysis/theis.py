...
def superposed_drawdown(wells: Sequence[PumpingWell], t_ft2d: float, s: float, x_ft, y_ft, t_days: float,
    ...
    `fn` is the single-well solution, defaulting to Theis; pass `Solution.drawdown` (see
    `analysis/solution.py`) to superpose a leaky-aquifer solution instead. Image wells for hydraulic
    boundaries are ordinary members of `wells`, possibly with a negative rate.
    """
    fn = fn or theis_drawdown
    ...
    for w in wells:
        ...
        total = total + fn(w.q_gpm, t_ft2d, s, r, t_days)
    return total
...
def drawdown_at_well(target: PumpingWell, wells: Sequence[PumpingWell], t_ft2d: float, s: float,
                     t_days: float, fn=None) -> DrawdownBreakdown:
    fn = fn or theis_drawdown
    self_ft = float(fn(target.q_gpm, t_ft2d, s, target.r_w_ft, t_days)) if target.q_gpm else 0.0
    contrib, dists = {}, {}
    for w in wells:
        ...
        dists[w.id] = d
        contrib[w.id] = float(fn(w.q_gpm, t_ft2d, s, max(d, w.r_w_ft), t_days))
...
class RadiusResult:
    ...
    center_xy: tuple
    per_azimuth_ft: list
    #: Azimuths where the aquifer ended before drawdown fell to the threshold, so the reported distance
    #: is the distance to the boundary and the cone is cut off rather than tapering out.
    truncated_azimuths: int = 0


def pumping_center(wells: Sequence[PumpingWell]) -> tuple[float, float]:
    """Rate-weighted centroid of the real pumping wells: the point a cone is reported about.

    Public because callers that add image wells must compute this from the real wells only and pass it
    in, or the images drag the centre away from the well field.
    """
...
def radius_at_drawdown(wells: Sequence[PumpingWell], t_ft2d: float, s: float, t_days: float,
                       threshold_ft: float, azimuths: int = 36, r_max_ft: float = 5.0e6, fn=None,
                       center: tuple | None = None, limit_fn=None) -> RadiusResult:
    ...
    `center` is passed explicitly when `wells` includes image wells, whose rates would otherwise drag
    the rate-weighted center away from the real wells and report the cone about the wrong point.

    `limit_fn(ux, uy) -> max_r_ft` bounds each ray, for a hydraulic boundary the cone cannot extend
    past: beyond a barrier the superposition keeps *increasing* with distance, so an unbounded search
    along that azimuth would find no edge at all or an absurd one.
    """
    cx, cy = center if center is not None else pumping_center(wells)
    radii = []
    truncated = 0
    ...
    for k in range(azimuths):
        ...
        reach = min(r_max_ft, limit_fn(ux, uy) if limit_fn else math.inf)
        if reach <= math.exp(lo):
            radii.append(0.0)
            continue
        hi = math.log(reach)
        ...
        def f(logr, ux=ux, uy=uy):
            r = math.exp(logr)
            return float(superposed_drawdown(wells, t_ft2d, s, cx + ux * r, cy + uy * r, t_days, fn)) - threshold_ft

        if f(hi) > 0:
            # Still above the threshold where the search had to stop.
            radii.append(reach)
            truncated += 1 if limit_fn and reach < r_max_ft else 0
            continue
        if f(lo) < 0:
            radii.append(0.0)
            continue
    ...
    arr = np.array(radii)
    return RadiusResult(threshold_ft, float(arr.max()), float(arr.mean()), float(arr.min()), (cx, cy),
                        radii, truncated)


def drawdown_grid(wells: Sequence[PumpingWell], t_ft2d: float, s: float, t_days: float,
                  half_extent_ft: float, n: int = 401, center: tuple | None = None, fn=None):
    ...
    return X, Y, superposed_drawdown(wells, t_ft2d, s, X, Y, t_days, fn)
